...
const ADMIN_ID = 'default'; // Restored hardcoded ID to recover data
...
export function useHeating() {
    ...
    const apiPost = useCallback(async <T>(endpoint: string, body: unknown): Promise<T> => {
    ...
    const apiPut = useCallback(async (endpoint: string, body: unknown): Promise<void> => {
        const res = await fetch(`${API_BASE}/${endpoint}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    const apiDelete = useCallback(async (endpoint: string, id: number): Promise<void> => {
    ...
    const forceProcess = useCallback(async (manual = false) => {
        ...
        // REMOVIDO: Checagem client-side pode impedir execução se o estado local estiver desatualizado
        // Deixar o backend decidir se há algo para processar
        /*
        if (!manual) {
        ...
        try {
            ...
            if (data.success && data.results) {
                ...
                if (sentCount > 0) {
                    ...
                    if (errors.length > 0) {
                        toast.error(`Erros: ${errors[0].message}`);
                    } else if (skipped.length > 0) {
                        // Translate common skip messages
                        const msg = skipped[0].message;
                        if (msg === 'All bots reached daily limit') {
                            toast.warning("Limite diário dos bots atingido!");
                        } else if (msg === 'Outside time window') {
                            toast.warning("Fora do horário configurado.");
                        } else if (msg === 'Waiting for interval') {
                            toast.info("Aguardando intervalo entre envios.");
                        } else {
                            toast.info(`Não enviado: ${msg}`);
                        }
                    } else {
                        toast.info("Nenhuma campanha rodando encontrada.");
                    }
                }
            } else if (manual) {
    ...
    // Automatic polling for processing
    useEffect(() => {
        const interval = setInterval(() => forceProcess(false), 30000);
        return () => clearInterval(interval);
    }, [forceProcess]);

    // Automatic polling for data sync (keep UI updated)
    useEffect(() => {
        const interval = setInterval(() => loadFromAPI(), 60000);
        return () => clearInterval(interval);
    }, [loadFromAPI]);
    ...
    }, [validateBotToken, loadFromAPI, bots, apiPost]);
