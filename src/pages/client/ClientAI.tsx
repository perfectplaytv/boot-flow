  ...
  return (
    <div className="space-y-4 sm:space-y-6 min-h-screen bg-[#09090b] p-3 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">IA + Voz</h1>
          <p className="text-gray-400 text-sm sm:text-base">Crie vozes personalizadas e processe áudios com IA</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <Button variant="outline" className="flex items-center gap-2 bg-[#1f2937] text-white border-none h-10 sm:h-auto" onClick={() => setModal({ type: 'upload' })}>
            <UploadCloud className="w-4 h-4" />
            <span className="hidden sm:inline">Upload Áudio</span>
            <span className="sm:hidden">Upload</span>
          </Button>
          <Button className="bg-[#7e22ce] hover:bg-[#6d1bb7] text-white flex items-center gap-2" onClick={() => setModal({ type: 'novaVoz' })}>
            <Plus className="w-4 h-4" />
            <span className="hidden sm:inline">Nova Voz IA</span>
            <span className="sm:hidden">Nova Voz</span>
          </Button>
        </div>
      </div>
      ...
      {tab === 'tts' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <Card className="md:col-span-2 bg-gradient-to-br from-purple-900/50 to-purple-800/30 border border-purple-700/40">
            <CardHeader>
              <CardTitle className="text-lg text-white">Estúdio de Voz</CardTitle>
      ...
      <div className="mb-6">
        <Card className="bg-gradient-to-br from-purple-900/50 to-purple-800/30 border border-purple-700/40">
          <CardHeader>
            <CardTitle className="text-lg text-white">Transcrições de Áudio</CardTitle>
            <p className="text-gray-400">Áudios processados com IA e resumos automáticos</p>
          </CardHeader>
          <CardContent className="space-y-4">
            {transcricoes.map(t => (
              <div key={t.id} className="bg-[#181e29] rounded-xl p-4 border border-purple-700/20 flex flex-col gap-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge className={t.status === 'processado' ? 'bg-green-700 text-green-200' : 'bg-yellow-700 text-yellow-200'}>{t.status}</Badge>
                  {t.sentimento && <Badge className={t.sentimento === 'negativo' ? 'bg-red-700 text-red-200' : t.sentimento === 'neutro' ? 'bg-blue-700 text-blue-200' : ''}>{t.sentimento}</Badge>}
                </div>
                <div className="text-gray-200 text-sm">"{t.texto}"</div>
                {t.resumo && <div className="bg-blue-900/40 rounded p-2 text-xs text-blue-200 mt-2">Resumo: {t.resumo}</div>}
                <div className="flex gap-2 mt-2">
