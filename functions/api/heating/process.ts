...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    try {
        // Reset daily counters if needed (Lazy reset based on UTC date)
        // Checks if last update was before today (UTC)
        await DB.prepare(`
            UPDATE heating_bots 
                ...
                if (campaign.last_sent_at) {
                    ...
                    if (timeSinceLast < randomInterval) {
                        results.push({
                            campaign: campaign.name,
                            status: 'skipped',
                            message: `Waiting for interval (${Math.round((randomInterval - timeSinceLast) / 1000)}s remaining)`
                        });
                        continue;
                    }
                }
